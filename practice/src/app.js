...
export default {
  ...
  data() {
    return {
      ...
      codeSecond:'',
      codeThird:'',
      codelist:['1-1101','22-222','3-3003'],
      linklist:['https://www.credamo.com/s/7bIJFnano/ cdmeb21RWQaI','https://www.credamo.com/s/jMNzuyano/ cdmeb21RWQaI','https://www.credamo.com/s/AnmMzmano/ cdmeb21RWQaI'],
      size: {},
  ...
  computed: {
    isNotOk() {
      return store.state.isNotOk
    },
    firstRoundFinished() {
      return store.state.firstRoundFinished
    },
    secondRoundFinished() {
      return store.state.secondRoundFinished
    },
    thirdRoundFinished() {
      return store.state.thirdRoundFinished
    },
    curr() {
  ...
  methods: {
    handleEnterFirst() {
      if(this.codeFirst.trim() == this.codelist[0]) {
    ...
    handleEnterSecond() {
      if(this.codeSecond.trim() == this.codelist[1]) {
        store.commit('updateIsLock', false)
        startTime(store, states)
        states.start()
        this.codeSecond = ''
    ...
    handleEnterThird() {
      if(this.codeThird.trim() == this.codelist[2]) {
        // store.commit('updateIsLock', false)
        // startTime(store, states)
        // states.start()
        this.codeThird = ''
