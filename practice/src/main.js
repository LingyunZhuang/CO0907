...
if (store.state.ifPractice === 1) {
  ...
  setTimeout(function () {
      ...
      localStorage.setItem('speedRun', store.state.speedRun)
      window.location.href = a
  }, 10*60*1000)//此处修改的时间是练习的时间
}
else {
  // 如果是正式页面，就10分钟后跳转
  const speedCondition = getUrlParameter('c')
  let nth_game = getUrlParameter('f')
  let simpleNormalDiffcult = []
  const speedRunPractice = localStorage.getItem('speedRun')
  if(speedRunPractice&&store.state.ifPractice !==1) {
    const speedStart = +speedRunPractice
    const time = speeds[speedStart-1]
    speeds[speedStart-2] = time/0.4
    speeds[speedStart] = time*0.4
    // console.log(speeds,'speeds======')
    // 使用示例
    const arrayLength = 3;
    const min = speedStart-1; // 随机数的最小值
    const max = speedStart+1; // 随机数的最大值
    simpleNormalDiffcult = generateUniqueRandomNumbers(min, max, arrayLength);
    store.commit('updateSimpleNormalDiffcult', simpleNormalDiffcult)
    store.commit('speedStart', simpleNormalDiffcult[store.state.curr])
    console.log('speedStart','-------', simpleNormalDiffcult[store.state.curr], store.state.curr)
  }
